...
void molecular_hydrogen_partition(double temp, double result[3]) {
    ...
    energy per molecule, the heat capacity per molecule at constant volume, and
    the adiabatic index
    */
...
void main() {
    ...
    fprintf(fptr,
            "# (0) Temperature (K) (1) Average energy (erg) (2) Heat capacity "
            "at constant vol (erg/K) (3) Adiabatic index gamma_1\n");
    while (logT <= 5) {
        Tgrid[i] = pow(10., logT);
